...
class _AddComplaintViewState extends State<AddComplaintView> {
                ...
                listener: (context, state) {
                  if (state is GovenmentLoading) {
                    ...
                    showDialog(
                      ...
                      builder: (c) => const Center(child: CircularProgressIndicator(color: AppColor.primaryColor,)),
                    );
                    ...
                    _showSelectionSheet(
                      ...
                      onSelected: (val) {
                        ...
                        final gov = _governmentsList!.data!.firstWhere((e) => e.name == val);
                        context.read<ComplaintTypeCubit>().getCoplaintsType(gov.id.toString());
                      },
                    );
                  } else if (state is GovenmentFetchFail) {
                     if (_isFetchingGovernments) {
              ...
              BlocListener<ComplaintTypeCubit, ComplaintTypeState>(
                ...
                child: GestureDetector(
                  onTap: () {
                    if (_selectedGovernment == null) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('يرجى اختيار الجهة الحكومية أولاً')),
                      );
                      return;
                    }
                    ...
                    if (_type != null) {
                       _showSelectionSheet(
                        ...
                        onSelected: (val) {
                          setState(() {
                            _selectedType = val;
                            final selectedType = _type!.data!.firstWhere((e) => e.name == val);
                            typeId = selectedType.id;
                          });
                        },
                      );
  ...
  Widget _buildFakeDropdown({required String text, required bool isPlaceholder}) {
    return Container(
      height: 52,
      padding: const EdgeInsets.symmetric(horizontal: 12),
      decoration: BoxDecoration(
  ...
  Widget _filesGrid() {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
         final file = selectedFiles[index];
        final isImage = file.path.endsWith('.png') || file.path.endsWith('.jpg') || file.path.endsWith('.jpeg');
        return Stack(
          children: [
            Container(
              decoration: BoxDecoration(
              ...
              child: isImage
                  ...
                  : const Center(child: Icon(Icons.insert_drive_file, size: 32)),
            ),
             Positioned(
              top: 4, right: 4,
              child: GestureDetector(
